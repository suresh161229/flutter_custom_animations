...
### 2. Use `const`
Because `custom_animation` evaluates the widget chain at build time, using `const` on your effects and child widgets allows Flutter to drastically optimize the rendering pipeline.
...
Currently, `custom_animation` focuses on entrance and state animations. For scroll-based animations, use `CustomAnimatedList` which automatically staggers the entrance of items as they are lazily built by the `ListView`.
